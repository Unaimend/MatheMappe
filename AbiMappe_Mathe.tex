...
\\	Beispiel:\\
	...
	Ein Vektor $\vec{a}$ wird mit einer Zahl $s$ aus $\mathbb{R}$ multipliziert, in dem seine Komponenten mit $s$ multipliziert werden.\\Beispiel
	...
	Das Sklalarprodukt ordnet je zwei Vektoren $\vec{a},\vec{b}$ ein Sklalar zu, ist dieses Skalar $0$ sind die Vektoren orthogonal zueinander, ist es $< 0$ sind sie parallel und gleichorientiert, ist es $> 0$, sind sie parallel und entgegengesetzt orintiert.\\\\Beispiel $\vec{a} \circ \vec{b}=a_1*b_1+a_2*b_2+a_3*b_3$.
	...
	Sind die Richtungsvektoren linear Abhaengig, sind die Gerade etweder Parralel oder Identisch, sind sie jedoch linear Unabhaengig
	muss geprueft werden ob sie einen Schnittpunkt besitzen oder windschief sind.
	\\\\Beispiel \textbf{Parralel}
	...
	\\Beispiel\\
	Fuer die Pruefung ob die Gerade Parralel sind siehe Lage Gerade Gerade.\\
	Seien die Gerade $g_1: \vec{x}= \vec{o_1}+r*\vec{a}$ und $ g_2: \vec{x}=\vec{o_2}+s*\vec{b} $ parralel.\\
